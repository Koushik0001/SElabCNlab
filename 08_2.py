...
import sys
import random
from turtle import color                   #This is used to genarate random numberes
...
def multiplot(x_axis1,y_axis1,x_axis2,y_axis2):
    ...
    ax2.set_ylabel(ylabel = "y = p(x) = Number of Occurences of k_number_of_Heads",color = "forestgreen")
    ax2.set_title(label = "Simulated Coin Toss Experiment",color = "limegreen")
...
class Coin_Toss():
    ...
    def toss(self):
        ...
        generator defined above and calculates the probability for each k (number of success) 
        and stores the probabilities in the 'ResultList' """
        for i in range(0,(2**self.numberOfToss)*int(sys.argv[3])):
            ...
            for j in range(0,self.numberOfToss):
                ...
                #this checks if result is present in the list if it is present status is set to 
                # 1 otherwise 0. For example if the second argument is 3/5 then the following 
                # list contains [0,1,2] now if the result of the random number generator is one 
                # of this three then status = 1 otherwise status = 0
...
try:
    ...
    print("Three arguments are needed to execute the program...")
    sys.exit()
except InvalidInput:
    ...
    print("The 1st argument should be at least 3 and the 2nd argument should be less than 1 and greater than zero...")
    sys.exit()
